...
const effectFrag = function() {
  ...
  float snoise3(vec3 v, float pos)
    {
  ...
  //Normalise gradients
    ...
    float deFactor = 42.0; //default noise factor (pos is dynamic noise factor)
    
  // Mix final noise value
    vec4 m = max(0.6 - vec4(dot(x0,x0), dot(x1,x1), dot(x2,x2), dot(x3,x3)), 0.0);
    m = m * m;
    
    return pos * dot( m*m, vec4( dot(p0,x0), dot(p1,x1),
                                  dot(p2,x2), dot(p3,x3) ) );
    }
...
const effectShader = function() {
  return glsl(`attribute vec3 position;
  attribute vec2 uv;
