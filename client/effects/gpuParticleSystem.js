...
THREE.GPUParticleSystem = function(options) {
	...
	self.particleNoiseTex = self.PARTICLE_NOISE_TEXTURE || textureLoader.load(document.querySelector('.texture1').dataset.src);
	self.particleNoiseTex.wrapS = self.particleNoiseTex.wrapT = THREE.RepeatWrapping;

	self.particleSpriteTex = self.PARTICLE_SPRITE_TEXTURE || textureLoader.load(document.querySelector('.texture2').dataset.src);
